...
const GateEmployeePage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [contractorsRes, factoriesRes] = await Promise.all([
    ...
    const onSubmit = async (data) => {
        try {
            setLoading(true);
            const response = await trucksAPI.register({
                ...data,
                gateId: 'gate-1', // يمكن تغييرها حسب البوابة
            });
                            ...
                            <Card title="آخر عربية مسجلة" className="bg-green-50 border-green-200">
                                <div className="space-y-2">
                                    ...
                                    <div className="text-sm space-y-1">
                                        ...
                                        <p><span className="font-medium">المقاول:</span> {lastTruck.contractor?.name}</p>
                                        <p><span className="font-medium">المصنع:</span> {lastTruck.factory?.name}</p>
                                        <p><span className="font-medium">الوقت:</span> {new Date(lastTruck.registeredAt).toLocaleTimeString('ar-EG')}</p>
                                    </div>
                                </div>
                            </Card>
                        )}
...
export default GateEmployeePage; 